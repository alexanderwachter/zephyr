		  ...
		  struct can_mcan_data *data)
{
	...
	if (cfg->sample_point_data) {
		...
		timing_data.phase_seg1 = cfg->prop_ts1;
		timing_data.phase_seg2 = cfg->ts2;
...
static void can_mcan_state_change_handler(struct can_mcan_data *data)
{
	/*TODO*/
	...
	do {
		if (can->ir & (CAN_MCAN_IR_BO | CAN_MCAN_IR_EP |
			...
			can->ir = CAN_MCAN_IR_BO | CAN_MCAN_IR_EP |
				  CAN_MCAN_IR_EW;
			can_mcan_state_change_handler(data);
		}
			...
			const struct zcan_filter *filter)
{
	...
	if (filter_nr == CAN_NO_FREE_FILTER) {
		LOG_INF("No free std filter left");
		return CAN_NO_FREE_FILTER;
	}
			       ...
			       const struct zcan_filter *filter)
{
	...
	if (filter_nr == CAN_NO_FREE_FILTER) {
		LOG_INF("No free ext filter left");
		return CAN_NO_FREE_FILTER;
	}
