...
typedef uint32_t (*can_get_core_clock_t)(const struct device *dev);
 ...
 * @retval 0 on error
 */
__syscall uint32_t can_get_core_clock(const struct device *dev);

static inline uint32_t z_impl_can_get_core_clock(const struct device *dev)
{
	const struct can_driver_api *api =
		(const struct can_driver_api *)dev->api;
